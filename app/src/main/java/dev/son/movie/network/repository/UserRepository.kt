...
class UserRepository(val api: UserAPI, private val userPreferences: UserPreferences) {
    ...
    fun getCurrentUser(): Observable<ApiResponse<User>> = api.getCurrentUser().subscribeOn(Schedulers.io())

    fun upDateUser(user: HashMap<String, Any>): Observable<ApiResponse<String>> = api.upDateUser(user).subscribeOn(Schedulers.io())
    fun getFavoriteList(): Observable<ApiResponse<List<Movie>>> = api.getFavoriteList().subscribeOn(Schedulers.io())
    fun getCommentedMovies(): Observable<ApiResponse<List<Movie>>> = api.getCommentedMovies().subscribeOn(Schedulers.io())
