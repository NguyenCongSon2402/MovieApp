...
class PlayMovieOfflineActivity : TrackingBaseActivity<ActivityMovieDetailsBinding>(),
    ...
    private val single: Boolean
        get() = intent.getBooleanExtra("single", false)

    private val movieID: String?
        get() = intent.extras?.getString("idMovie")
    private val movieName: String?
        get() = intent.extras?.getString("nameMovie")
    private val uriMovie: String?
        get() = intent.extras?.getString("uriMovie")
    private val imgMovie: String?
        get() = intent.extras?.getString("imgMovie")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.e("NAME", "${movieName}  + ${movieID} + ${uriMovie} ")
        showLoader(true)
        fetchData()
        setUpPlayVideo()
        setupUI()
...
//                    ).show()
            ...
            when (it.movieById) {
                is Success -> {
                    if (it.movieById.invoke().data.id != null) {
                        movie = it.movieById.invoke().data
                        Log.e("AAAAAA", "${movie}")
                        ...
                        views.header.overviewText.text = movie.description
                        views.header.yearText.text = movie.releaseYear
                        views.header.runtimeText.text = movie.duration
                        showLoader(false)
                    }
...
//                    ).show()
    ...
    private fun fetchData() {
        ...
        if (!uriMovie.isNullOrEmpty()) {

//            views.thumbnail.container.hide()
            val (firstPart, secondPart) = movieID!!.split("_").let {
                it.getOrElse(0) { "" } to it.getOrElse(1) { "" }
            }
//            initFullscreenDialog()
//            Log.e("NAME", "${movieName}  + ${movieID} + ${uriMovie} ")
//            views.header.titleText.text = movieName
//            media = MediaItem.Builder().setUri(uriMovie).setMediaId(movieID!!)
//                .setMediaMetadata(
//                    MediaMetadata.Builder().setTitle(imgMovie)
//                        .setDisplayTitle(movieName)
//                        .build()
//                ).build()
...
//            playMovie(uriMovie!!)
            showLoader(false)
            homeViewModel.handle(HomeViewAction.getMovieRate(firstPart))
            homeViewModel.handle(HomeViewAction.movieById(firstPart))
            homeViewModel.handle(HomeViewAction.getCommentByMovie(firstPart))
            homeViewModel.handle(HomeViewAction.getMoviesRecommendation(firstPart))
        }
...
//get coins
        ...
        lifecycleScope.launch {
            userPreferences.user.collect {
                currentUser = it ?: User()
                Log.e("USER", currentUser.id.toString())
            }
        }
